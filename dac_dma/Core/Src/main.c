...
volatile uint32_t callbck_cnt = 0;
volatile uint32_t cb_cplt;
volatile uint32_t cb_half;

uint16_t dma_buffer[2 * DMA_BUFFER_SIZE];

float angle = 0;
float angle_change = 10 * (2 * M_PI / SAMPLE_FREQ);
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  if (now >= next_tick) {
	  	 ...
	  	 printf("Tick %lu (loop = %lu cb = %lu half = %lu cplt = %lu)\n\r", now / 1000, loop_count, callbck_cnt, cb_half, cb_cplt);
